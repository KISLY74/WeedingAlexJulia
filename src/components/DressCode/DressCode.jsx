import "./DressCode.scss";

export default function DressCode() {

    return <>
        <section className="dresscode">
            <h1>Дресскод</h1>
            <div className="dresscode-content">
                <p>Мы очень ждем и с удовольствием готовимся к нашему незабываемому дню!</p>
                <p>Поддержите нас вашими улыбками и объятиями, а также красивыми нарядами в классическом стиле.</p>
                <p>Для нас главное - Ваше  присутствие! Но мы будем рады, если в своих нарядах вы поддержите цветовую гамму нашей свадьбы.</p>
                <p>Необязательно одевать наряды как на фото, это просто пример.</p>
                <p className="circle-black"></p>
            </div>
            <div className="photo">
                    <img src="/images/dresscode/wm1.png" alt="" />
                    <img src="/images/dresscode/wm2.png" alt="" />
                    <img src="/images/dresscode/wm3.png" alt="" />
                    <img src="/images/dresscode/wm4.png" alt="" />
                    <img src="/images/dresscode/m1.png" alt="" />
                    <img src="/images/dresscode/m2.png" alt="" />
            </div>
        </section>
    </>
}